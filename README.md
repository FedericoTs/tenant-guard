...
| `identity-trust` *(runtime)* | the caller can **forge the identity** your policies authorize from, or **write the table that grants it** | every other guard asks "given a correct identity, is the data scoped?" — this asks whether the identity itself is controllable: a policy reading `user_metadata` (which the *user* can rewrite) is defeated by forging that claim; a callable `SECURITY DEFINER` that sets your tenant GUC from an argument is a "become any tenant" primitive; and a `memberships` table the caller can write makes a *flawless* policy bypassable — insert yourself into any org and the policy hands over that org's data, legitimately |
