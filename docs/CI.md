...
|---|---|---|
| `command` | `run` | `run` = static guards only. `all` = static + every runtime proof. Or one guard: `prove`, `drift`, `anon-reads`, `anon-writes`, `identity`, `rpc`, `views`, `storage`, `realtime`, `schemas`, `pooler`, `shadows`, `oracles`, `caps` |
