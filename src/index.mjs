...
/**
 * Run the column-exposure check (async; needs a database URL + `pg`).
 * Proves the read: fails only when `anon` gets a real non-null value out of a
 * sensitive column, on a relation with no tenant column — the table no tenancy
 * guard covers, because there are no tenants to leak across.
 */
